...
class Pinyin
{
     ...
     * 获取全部拼音
     * @param $chinese 中文字符串
     * @param string $delimiter 空格
     * @param int $length 截断长度
     * @param string $charset 字符编码
     * @return bool|mixed|string
     */
    public function py($chinese, $delimiter = '', $length = 0, $charset = 'utf-8')
    {
        if ($charset != 'gb2312') {
            $chinese = $this->utf8_to_gb($chinese);
        }
     ...
     * @return string
     */
    public function py_initial($chinese, $charset = 'utf-8')
    {
        if ($charset != 'gb2312') $chinese = $this->utf8_to_gb($chinese);
        $result = '';
